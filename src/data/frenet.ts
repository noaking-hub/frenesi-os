...
// ── Faturado sem rastreio: o buraco que a Frenet não deixa fechar sozinho ──
//
// Etiqueta comprada na Yampi chega pelo track_code; etiqueta feita direto no
// painel da Frenet não chega por lugar NENHUM — cinco pedidos ficaram
// "faturado" por dias com a etiqueta impressa e o cliente sem aviso. A porta
// de listagem foi SONDADA em 19/08 (/shipping/order, /shipping/orders,
// /shipping/order/list, /order/list, /shipping/label/list): 404 em todas —
// a API pública é só cotação e rastreio de código conhecido. Logo, descoberta
// automática não existe para a Frenet; o processo é informar o código na
// Yampi ao faturar. Esta função vigia o buraco: lista quem está faturado sem
// código para o relatório e para o alerta de expedição.

export interface SondagemEnviosFrenet {
  /** Pedidos pagos e faturados sem rastreio — etiqueta possivelmente no painel. */
  pendentes: string[]
}

export async function sondarEnviosDaFrenet(): Promise<SondagemEnviosFrenet> {
  if (!frenetConfigurada() || !supabaseConfigurado()) return { pendentes: [] }
  ...
  const { data } = await sb
    ...
    .limit(20)
  return {
    pendentes: ((data ?? []) as { id: string; shopify_numero: string | null }[]).map(
