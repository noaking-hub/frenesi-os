-- "Embalagem e insumos" era um número só; o operador quer enxergar frasco,
-- válvula, etiqueta, caixa e plástico bolha separados. A coluna `insumos`
-- continua guardando a SOMA — todo o cálculo de preço lê dela, e uma coluna
-- derivada que diverge dos detalhes seria uma segunda verdade.

alter table parametros_precificacao
  add column frasco         numeric(10, 2) not null default 0,
  add column valvula        numeric(10, 2) not null default 0,
  add column etiqueta       numeric(10, 2) not null default 0,
  add column caixa          numeric(10, 2) not null default 0,
  add column plastico_bolha numeric(10, 2) not null default 0;

-- Vigências antigas: o total conhecido entra como frasco até o operador
-- detalhar — a soma não muda, o preço calculado não muda.
...
begin
  ...
  if least(p_intermediador_fixo, p_frasco, p_valvula, p_etiqueta, p_caixa,
           p_plastico_bolha, p_frete_subsidio, p_antifraude) < 0 then
  ...
  insert into parametros_precificacao (
    ...
    insumos, frasco, valvula, etiqueta, caixa, plastico_bolha,
    ...
    round(p_frasco + p_valvula + p_etiqueta + p_caixa + p_plastico_bolha, 2),
    round(p_frasco, 2), round(p_valvula, 2), round(p_etiqueta, 2),
    round(p_caixa, 2), round(p_plastico_bolha, 2),
